// LDK implementation of the Lightning Network interface
// Uses the Lightning Development Kit to provide a full Lightning Network node
...
#[cfg(feature = "ldk")]
use lightning::{
    chain::keysinterface::{KeysManager, InMemorySigner},
    ln::{
        channelmanager::{ChannelManager, ChannelManagerReadArgs},
        peerhandler::{PeerManager, SocketDescriptor},
        msgs::SocketAddress,
    },
    util::{
        config::UserConfig,
...
/// LDK implementation of Lightning Network interface
pub struct LdkLightningImplementation {
    /// Configuration
...
impl LdkLightningImplementation {
    /// Create a new LDK Lightning implementation
    ...
    fn ensure_initialized(&self) -> LightningResult<()> {
        ...
        if !*initialized {
            println!("Initializing LDK Lightning implementation...");
            ...
            *initialized = true;
            println!("LDK Lightning implementation initialized");
        }
    ...
    /// Get the LDK data directory
    fn get_ldk_data_dir(&self) -> PathBuf {
        let base_dir = self.config.bitcoin_data_dir.clone()
...
impl LightningInterface for LdkLightningImplementation {
