...
// Provides a unified interface for Bitcoin operations with support for both
// Python (python-bitcoinlib) and Rust (rust-bitcoin/BDK) implementations.
//
...
//
// 1. Gradual migration from Python to Rust implementation
// 2. Testing core business logic against multiple implementations
// 3. Runtime selection of implementation based on features or configuration
// 4. Clear separation of interface (this file) from implementation details
...
pub enum BitcoinImplementationType {
    /// Use the Python bitcoin implementation (python-bitcoinlib)
    Python,
...
/// Factory to create the appropriate Bitcoin implementation
/// 
/// This function implements the Factory pattern to create the appropriate 
/// implementation based on the requested type and available features.
/// It handles fallback behavior when the requested implementation is not available.
pub fn create_bitcoin_interface(
    ...
    match implementation_type {
        ...
        BitcoinImplementationType::Python => {
            use crate::bitcoin::python::PythonBitcoinImplementation;
            Arc::new(PythonBitcoinImplementation::new(config))
        }
        
        #[cfg(feature = "rust-bitcoin")]
        BitcoinImplementationType::Rust => {
            use crate::bitcoin::rust::RustBitcoinImplementation;
            Arc::new(RustBitcoinImplementation::new(config))
        }
        
        #[cfg(not(feature = "rust-bitcoin"))]
        BitcoinImplementationType::Rust => {
            eprintln!("Warning: Rust Bitcoin implementation requested but not available. Falling back to Python implementation.");
            use crate::bitcoin::python::PythonBitcoinImplementation;
            Arc::new(PythonBitcoinImplementation::new(config))
        }
        ...
        BitcoinImplementationType::Python => {
            eprintln!("Warning: Python Bitcoin implementation requested but not available. Falling back to Rust implementation.");
            use crate::bitcoin::rust::RustBitcoinImplementation;
            Arc::new(RustBitcoinImplementation::new(config))
        }
    }
...
/// Helper function to get current implementation based on configuration
/// 
/// Examines the configuration to determine which implementation should be used
...
pub fn get_current_bitcoin_interface(config: &crate::config::Config) -> Arc<dyn BitcoinInterface> {
    let implementation_type = if config.use_rust_bitcoin {
        BitcoinImplementationType::Rust
    } else {
        BitcoinImplementationType::Python
    };
    
    create_bitcoin_interface(implementation_type, config)
}
...
mod tests {
    ...
    fn test_interface_creation() {
        ...
        let python_impl = get_current_bitcoin_interface(&config);
        assert_eq!(python_impl.implementation_type(), BitcoinImplementationType::Python);
